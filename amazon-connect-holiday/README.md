...
```bash
amazon-connect-holiday$ sam build
```
...
```bash
amazon-connect-holiday$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
amazon-connect-holiday$ sam local start-api
amazon-connect-holiday$ curl http://localhost:3000/
```
...
```bash
amazon-connect-holiday$ sam logs -n HelloWorldFunction --stack-name amazon-connect-holiday --tail
```
...
```bash
amazon-connect-holiday$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name amazon-connect-holiday
```
